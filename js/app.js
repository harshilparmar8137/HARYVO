...
products.forEach((p,i)=>{
 ...
 <div class="product-body"><div class="type">${p[2]}</div><h3>${p[0]}</h3><p>${p[3]}</p><div class="product-card-price">${displayPrices[p[1]]||""}</div><div class="product-actions">
...
function botReply(q){const s=String(q).toLowerCase();if(/price|cost|rate|₹|rupee/.test(s)){return "I can help with HARYVO product pricing. Please ask with a product name or code, for example: HARYVO S4 or HARYVO-000002."}if(/s2|000001/.test(s))return "HARYVO S2 Controller — HARYVO-000001 — ₹1,899.";if(/s4|000002/.test(s))return "HARYVO S4 Controller — HARYVO-000002 — ₹2,199.";if(/s8|000003/.test(s))return "HARYVO S8 Controller — HARYVO-000003 — ₹2,699.";if(/hvac|000024/.test(s))return "HARYVO HVAC Monitor — HARYVO-000024 — ₹2,099.";if(/curtain|000018/.test(s))return "HARYVO Smart Curtain Controller + Motor — HARYVO-000018 — ₹6,499.";return "I’m having trouble reaching HARYVO AI right now. Please try again in a moment, or ask me about a HARYVO product code."}
async function sendChat(){const i=document.getElementById("chatInput"),q=i.value.trim();if(!q)return;i.value="";addMsg(q,"user");addMsg("Thinking…","bot");const t=document.getElementById("messages").lastElementChild;const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),15000);try{const r=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:q}),signal:controller.signal});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"AI service unavailable");t.textContent=d.reply||botReply(q)}catch(e){t.textContent=botReply(q)}finally{clearTimeout(timer)}}
